...
import { loginUser, refreshUserToken, invalidateRefreshToken } from "@api/auth/auth.ts";
...
router.post("/refresh", async (ctx) => {
  await withRefreshAuth(
    ...
    async () => {
      ...
      const { accessToken, refreshToken, userResponse } = await refreshUserToken(oldToken, userId);
