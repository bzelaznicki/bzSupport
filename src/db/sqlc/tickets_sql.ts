...
export async function getNextTicketNumber(
  sql: Sql,
  args: GetNextTicketNumberArgs,
): Promise<GetNextTicketNumberRow | null> {
  const rows = await sql.unsafe(getNextTicketNumberQuery, [args.tenantId])
    .values();
  if (rows.length !== 1) {
    return null;
  }
...
): Promise<CreateTicketRow | null> {
  const rows = await sql.unsafe(createTicketQuery, [
    args.id,
    args.tenantId,
    args.publicId,
    args.subject,
...
export async function listTickets(
  ...
  return (await sql.unsafe(listTicketsQuery, [args.tenantId]).values()).map(
    (row) => ({
      id: row[0],
      tenantId: row[1],
      publicId: row[2],
...
export async function getTicket(
  sql: Sql,
  args: GetTicketArgs,
): Promise<GetTicketRow | null> {
  const rows = await sql.unsafe(getTicketQuery, [args.tenantId, args.id])
    .values();
  if (rows.length !== 1) {
    return null;
  }
